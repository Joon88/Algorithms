...
// O(VE) time and O(V+E) space for storing the graph - great for detecting -ve weight cycles
public class BellmanFord {
	...
	public class Node{
		...
		public Node(int i) {
			id = i;
			// subtracting 100, coz sometime while relaxing, we may add some weight to the max_value, making it -ve in 2's complement
	...
	public void runBellmanFord(Graph g, Node src) {
		...
		for(int i = 0 ; i < g.graph.size()-1 ; i++) {    // O(V)
			boolean atleastOneEdgeRelaxed = false;
			for(Edge e : edges) {                        // O(E)
				atleastOneEdgeRelaxed = atleastOneEdgeRelaxed || relaxEdge(e);   // O(1)
			}
			if(!atleastOneEdgeRelaxed)                  // Here itself we can say no -ve weight cycles present
				break;
		}
